...
from .transformer import MERUTransformerTextEncoder
...
class CLIPBaseline(nn.Module):
    ...
    def __init__(
        self,
        visual: nn.Module,
        textual: MERUTransformerTextEncoder,
        embed_dim: int,
    ...
    def forward(self, images: torch.Tensor, tokens: list[torch.Tensor]) -> dict[str, torch.Tensor]:
        """
        Args:
            images: Image batch in BCHW format, with pixel values in `[0, 1]`.
            tokens: List of tensors, each containing text tokens. Tensors may have
        ...
        loss = 0.5 * (F.cross_entropy(image_logits, targets) + F.cross_entropy(text_logits, targets))
        output_dict = {
            "loss": loss,
