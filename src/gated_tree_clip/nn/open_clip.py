...
class CLIPOrigin(nn.Module):
    ...
    def __init__(
        ...
        text_cfg: CLIPTextCfg = CLIPTextCfg(),
        act_layer: Callable = nn.GELU,
        ...
        self.visual = _build_vision_tower(embed_dim, vision_cfg, act_layer, cast_dtype)

        text = _build_text_tower(embed_dim, text_cfg, act_layer, cast_dtype)
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        x, _ = x[torch.arange(x.shape[0]), text.argmax(dim=-1)], x
...
def convert_weights_to_lp(model: nn.Module, dtype=torch.float16):
    ...
    def _convert_weights(layer):
        ...
        if isinstance(layer, (CLIPOrigin, OpenCLIPTextTransformer)):
...
def resize_pos_embed(state_dict, model, interpolation: str = "bicubic", antialias: bool = True):
    ...
    if old_pos_embed is None or not hasattr(model.visual, "grid_size"):
        return
    grid_size = to_ntuple(model.visual.grid_size, n=2)
    ...
    old_grid_size = to_ntuple(int(math.sqrt(len(pos_emb_img))), n=2)
